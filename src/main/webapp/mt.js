...
function mtScore(entry_id) {
    ...
    var url = 'http://www.akaruiheya.com/mt/mt-cp.cgi';
    ...
    xh.send( '__mode=vote&blog_id=23&f=scored,count&jsonp=mtScore_cb&id=' + entry_id);
    return false;
}
...
function mtUpdateScores() {
    ...
    xh.setRequestHeader( 'Content-Type', 'application/x-www-form-urlencoded' );
    xh.send( '__mode=score&blog_id=23&f=scored,count&jsonp=mtScore_cb&id=' + entry_ids);
    return false;
}
...
function favorite(evt, entry_id, score_span) {
    ...
    xh.setRequestHeader( 'Content-Type', 'application/x-www-form-urlencoded' );
    xh.send( '__mode=vote&blog_id=23&f=scored,sum&jsonp=favorite_cb&id=' + entry_id);
    var span = document.getElementById(score_span);
    if (span) span.innerHTML = '<img src="http://www.akaruiheya.com/mt/mt-static/images/indicator.white.gif" height="10" width="10" alt="favoriting..." />';
...
function script_follow(id) {
    ...
    var xh = mtGetXmlHttp();
    if (!xh) return false;
    xh.open('POST', 'http://www.akaruiheya.com/mt/mt-cp.cgi', true);
    xh.onreadystatechange = function() {
        if ( xh.readyState == 4 ) {
            if (xh.status && ( xh.status != 200 ) ) {
                // error - ignore
    ...
    if (span) span.innerHTML = '<img src="http://www.akaruiheya.com/mt/mt-static/images/indicator.white.gif" height="10" width="10" alt="Following..." />';
}
...
function script_leave(id) {
    ...
    var span = document.getElementById('following-status');
    if (span) span.innerHTML = '<img src="http://www.akaruiheya.com/mt/mt-static/images/indicator.white.gif" height="10" width="10" alt="Leaving..." />';
}
...
var mtCookieName = "mt_blog23_user";
var mtCookieDomain = ".akaruiheya.com";
var mtCookiePath = "/";
...
function mtFetchUser(cb) {
    ...
    if ( ( cb == 'mtSetUser' ) && mtGetUser() ) {
        ...
        script.src = 'http://www.akaruiheya.com/mt/mt-comments.cgi?__mode=session_js&blog_id=23&jsonp=' + cb + '&ts=' + ts;
...
function mtCommentOnSubmit(f) {
    if (!mtRequestSubmitted) {
        ...
        if (f.armor)
            f.armor.value = '415eb75850219664ef8621aff647c836fb26a66f';
...
function mtSignIn() {
    ...
    var url = 'http://www.akaruiheya.com/mt/mt-cp.cgi?__mode=login&blog_id=23';
    if (is_preview) {
        if ( document['comments_form'] ) {
            var entry_id = document['comments_form'].entry_id.value;
            url += '&entry_id=' + entry_id;
        } else {
            url += '&return_url=http%3A%2F%2Fwww.akaruiheya.com%2F';
        }
    } else {
...
function mtSignOut(entry_id) {
    ...
    var doc_url = document.URL;
    doc_url = doc_url.replace(/#.+/, '');
    var url = 'http://www.akaruiheya.com/mt/mt-cp.cgi?__mode=logout&blog_id=23';
    if (is_preview) {
        if ( document['comments_form'] ) {
            var entry_id = document['comments_form'].entry_id.value;
            url += '&entry_id=' + entry_id;
